 ...
 * @description Create Movie Comment
 * @access Public
 */
 ...
 * @access Public
 */
const getMovie = async (req: Request, res: Response): Promise<Response | void> => {
    const { movieId } = req.params;
 ...
 * @access Public
 */
const updateMovieComment = async (req: Request, res: Response): Promise<Response | void> => {
    const error = validationResult(req);
    if (!error.isEmpty()) {
        console.log(error);
        return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.BAD_REQUEST));
    ...
    try {
        const data = await MovieService.updateMovieComment(movieId, commentId, userId, commentUpdateDto);
